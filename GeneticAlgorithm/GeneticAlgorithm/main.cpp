...
#define N_GENERATION 40
//#define CROSSOVER_RATE_PERCENT 20
#define MUTATE_RATE_PERMILLAGE 200  
...
void initPerms(short *vectors[])
{
	...
	base_perm[0] = 0;
	for (i = 1; i < DNA_LENGTH; i++) {
		while (temp = rand() % (DNA_LENGTH - 1) + 1) {
			for (j = 0; j < i; j++) {
				if (temp == base_perm[j]) {
					break;
				}
...
void select()
{
	...
	//for (i = 0; i < POPULATION; i++) {
	//	G_parents_fitness[i] = G_parents_fitness[i] / sum;
	//	G_children_fitness[i] = G_children_fitness[i] / sum;
	//}
...
int savePerm(FILE *fp, short *perm) {
	if (NULL == fp) {
